...
img_number = 4760  ##enter image number here 


testimg_root = 'C:/Users/sheyd/OneDrive/Desktop/Visidon/image_for_final_testing/'  ## test image path
test_img = tensor(numpy.asarray(Image.open(testimg_root + str(img_number) +'_input.png')))
...
test_tar_py = numpy.asarray(Image.open(testimg_root + str(img_number) +'_target.png'))/255
...
"visual comparison"
inp_img = numpy.asarray(Image.open(testimg_root + str(img_number) +'_input.png'))
f, axarr = plt.subplots(1,3)
axarr[0].imshow(inp_img)
axarr[1].imshow(test_tar_py)
axarr[2].imshow(final_res)
